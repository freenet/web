...
pub enum ArmorableError {
    ...
    Serialization(#[from] Box<dyn std::error::Error>),
...
pub trait Armorable {
    // Convert the object to Vec<u8>
    fn to_bytes(&self) -> Result<Vec<u8>, Box<dyn std::error::Error>>;

    // Convert Vec<u8> to the object
    ...
    fn from_base64_armored(armored: &str) -> Result<Self, ArmorableError>
    where
        Self: Sized,
    {
        let label = extract_label(armored)?;
        if label != struct_label::<Self>() {
            return Err(ArmorableError::Io(io::Error::new(
                io::ErrorKind::InvalidData,
                "Armor label does not match",
            )));
        }
        let base64_data = extract_base64(armored, &label)?;
        let data = general_purpose::STANDARD.decode(&base64_data)?;
        let object = Self::from_bytes(&data)?;
        Ok(object)
    }
    ...
    fn from_naked_base64(base64: &str) -> Result<Self, ArmorableError>
    where
        Self: Sized,
    {
    ...
    fn from_file<P: AsRef<Path>>(path: P) -> Result<Self, ArmorableError>
    where
        Self: Sized,
    {
        ...
        for line in reader.lines() {
            ...
            if line.starts_with(&format!("-----BEGIN {}", label)) {
                if found_block {
                    return Err(ArmorableError::Io(io::Error::new(
                        io::ErrorKind::InvalidData,
                        "Multiple blocks with the same label found",
                    )));
                }
                in_block = true;
                found_block = true;
        ...
        if !found_block {
            return Err(ArmorableError::Io(io::Error::new(
                io::ErrorKind::InvalidData,
                "No block with the specified label found",
            )));
        }
...
// Marker trait for types that implement Serialize and Deserialize
pub trait SerializableArmorable: Serialize + for<'de> Deserialize<'de> {}

// Provide a default implementation for SerializableArmorable types
impl<T> SerializableArmorable for T where T: Serialize + for<'de> Deserialize<'de> {}

// Provide a default implementation for Armorable for SerializableArmorable types
impl<T> Armorable for T
where
    T: SerializableArmorable,
{
    fn to_bytes(&self) -> Result<Vec<u8>, Box<dyn std::error::Error>> {
...
// Helper function to get the label from the struct name
fn struct_label<T>() -> String {
    let name = type_name::<T>().split("::").last().unwrap();
    name.chars()
        .flat_map(|c| if c.is_uppercase() { vec![' ', c] } else { vec![c] })
        .collect::<String>()
        .trim()
        .to_uppercase()
}
