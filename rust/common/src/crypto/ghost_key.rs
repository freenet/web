...
pub fn generate_ghostkey(delegate_signing_key_pem: &str) -> Result<(String, String), CryptoError> {
    let delegate_signing_key_bytes = extract_base64_from_armor(delegate_signing_key_pem, "DELEGATE SIGNING KEY")?;
    ...
    let field_bytes = FieldBytes::from_slice(delegate_signing_key_bytes.as_bytes());
